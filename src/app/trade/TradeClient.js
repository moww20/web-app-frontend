...
export default function TradeClient() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("web3dex:mode-change", onMode)
    return () => window.removeEventListener("web3dex:mode-change", onMode)
  }, [])
            ...
            <div className="inline-flex items-center gap-1 hairline rounded-full px-2 py-1">
              {[
                ...
                <button key={r.label}
                  ...
                  onClick={() => { setActiveRange(r.label); window.dispatchEvent(new CustomEvent('web3dex:chart-range', { detail: { hours: r.hours } })) }}>
                  {r.label}
                </button>
              ))}
            </div>
            <div className="inline-flex items-center">
              <div className="inline-flex items-center hairline rounded-full mr-2">
                <button className={`px-2 py-1 rounded-l-full hover:bg-white/5 ${scaleMode==='linear'?'bg-white/10':''}`} title="Linear"
                  onClick={() => { setScaleMode('linear'); window.dispatchEvent(new CustomEvent('web3dex:chart-scale', { detail: { mode: 'linear' } }))}}>↗</button>
                <button className={`px-2 py-1 rounded-r-full hover:bg-white/5 ${scaleMode==='log'?'bg-white/10':''}`} title="Log"
                  onClick={() => { setScaleMode('log'); window.dispatchEvent(new CustomEvent('web3dex:chart-scale', { detail: { mode: 'log' } }))}}>∿</button>
              </div>
              <div className="relative inline-flex items-center hairline rounded-full bg-white/5 hover:bg-white/10 transition">
                <select
                  ...
                  value={priceMode}
                  onChange={(e)=> { setPriceMode(e.target.value); window.dispatchEvent(new CustomEvent('web3dex:chart-priceMode', { detail: { kind: e.target.value } })) }}
                >
