...
export default function AddLiquidityCard({
  ...
  const getPrice = useCallback((sym) => (tokenPrices && sym ? (typeof tokenPrices[sym] === 'number' ? tokenPrices[sym] : 0) : 0), [tokenPrices])
  const getBalance = useCallback((sym) => (tokenBalances && sym ? (typeof tokenBalances[sym] === 'number' ? tokenBalances[sym] : 0) : 0), [tokenBalances])
  ...
  }, [amountA, tokenA, tokenB, tokenPrices, amountB, deriveAmountB])
