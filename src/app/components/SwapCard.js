...
export default function SwapCard({
  ...
  const SETTINGS_KEY = "web3dex.settings"
  ...
  const setModeAndEmit = (next) => {
    setMode(next)
    try {
      window.dispatchEvent(new CustomEvent("web3dex:mode-change", { detail: next }))
    } catch {}
  }
  ...
  const deriveBuyAmount = useCallback((sellAmt, sellSym, buySym) => {
  ...
  }, [sellAmount, sellToken, buyToken, deriveBuyAmount])
