...
export default function PoolsClient() {
  ...
  const rows = Array.from({ length: 6 }).map((_, i) => (
    <div key={i} className="grid grid-cols-8 items-center px-4 py-3 hairline-b last:hairline-b-0 cursor-pointer hover:bg-white/3" onClick={() => setIsDetailsOpen(true)}>
      <div className="col-span-2 min-w-0">TokenA / TokenB</div>
      <div className="min-w-0">$ —</div>
      <div className="min-w-0">APR —</div>
      <div className="min-w-0">Liquidity —</div>
      <div className="min-w-0">Voting APR —</div>
  ...
  return (
    <main className="min-h-[80vh] max-w-7xl mx-auto px-6 py-24">
      ...
      <section className="glass hairline rounded-2xl overflow-hidden">
        <div className="grid grid-cols-8 px-4 py-3 text-sm text-[--color-muted] hairline-b bg-white/5">
