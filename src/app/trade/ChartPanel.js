  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const res = await fetch(
          "https://api.coingecko.com/api/v3/coins/ethereum/market_chart?vs_currency=usd&days=1&interval=hourly",
          {
            ...
            headers: {
              ...
              'User-Agent': 'Web3-DEX-Frontend/1.0.0'
            }
          }
        )
        ...
        console.log('ETH price data loaded successfully')
        ...
        console.warn('Failed to load ETH price data:', error.message)

        // Provide fallback data with a simple upward trend
        ...
        for (let i = 24; i >= 0; i--) {
          ...
          const price = basePrice + Math.sin(i / 4) * 100 + Math.random() * 50
          fallbackPoints.push({ time, value: Math.max(price, 2000) })
        }

        areaSeries.setData(fallbackPoints)
    ...
    window.addEventListener('web3dex:chart-range', onRange)
    window.addEventListener('web3dex:chart-scale', onScale)
    window.addEventListener('web3dex:chart-priceMode', onPriceMode)

    return () => {
      window.removeEventListener('web3dex:chart-range', onRange)
      window.removeEventListener('web3dex:chart-scale', onScale)
      window.removeEventListener('web3dex:chart-priceMode', onPriceMode)
      chart.remove()
    }
  }, [plotHeight, useMock])
